...
# def load_models():
#     extractor = models.Extractor(model="static/ru_core_news_sm/ru_core_news_sm-3.0.0")
...
@st.cache_data()
def load_data():
    phras_df = pd.read_csv("static/prep_phrases_db.csv")
    synt_df = pd.read_csv("static/syntaxemes.csv")
    base_prep = json.loads(open("static/base_prep.json").read())
    label_definitions = json.loads(open("static/definitions.json").read())
    semantic_df = {
        "Синонимы": pd.read_csv("static/synonyms.csv"),
        "Антонимы": pd.read_csv("static/antonyms.csv"),
    }

    return phras_df, synt_df, label_definitions, base_prep, semantic_df


@st.cache_data()
def load_configs():
    formatter = json.loads(open("static/format.json").read())
    default = json.loads(open("static/default.json").read())
...
st.header(":book:")
prep = st.selectbox(
    "Выберите предлог из списка, чтобы просмотреть его паспорт", all_preps
)
...
if prep:
    ...
    variants = ", ".join([f"`{p.upper()}`" for p in base_prep[prep]["variants"]])
        ...
        for l in prep_labels:
            with st.expander(f"{l}"):
                ...
                for _, row in short_df.iterrows():
                    st.write(f"### {formatter[row.case.lower()].capitalize()} падеж:")
...
st.header(":mag_right:")
with st.expander("Нажмите, чтобы сформировать запрос в Банк предложных конструкций"):
    ...
    with st.form(key="query"):
        ...
        for col in default["query_values"]:
            ...
            query[col] = st.multiselect(
                ...
                options=sorted(phras_df[col].dropna().unique().tolist()),
                default=d,
            )
    ...
    if submit:
        query = " and ".join(
            f"{key} in {val}" for key, val in query.items() if len(val)
        )
        ...
        tmp_link = utils.get_table_download_button(
            out_df, "prep_phrases_query.csv", "Cкачать"
        )
...
#     text = st.text_area(
#         label="Введите текст, чтобы извлечь предложные конструкции",
